...
@MultipartConfig /* Essa anotação é para aceitar upload de fotos */
		...
		try {
			...
			if (acao != null && !acao.isEmpty() && acao.equalsIgnoreCase("deletar")) {
				...
				response.getWriter().write(json);
					...
					// Redireciona para a tela de usuário e mantém os dados na tela
		...
		try {
			...
			if ( request.getPart("fileFoto") != null ) {
				...
				if ( part.getSize() > 0) {
					...
					//feito o split part.getContentType().split("\\/")[1] para pegar a extensaõ da foto
					String imagemBase64 = "data:image/" + part.getContentType().split("\\/")[1] + ";base64," + new Base64().encodeBase64String(foto);
					
					modelLogin.setFotoUser(imagemBase64);
					modelLogin.setExtensaoFotoUser(part.getContentType().split("\\/")[1]); /*Pega somente a extensão png*/
					
				}
				
			}
			
			// Se ja existe mesmo login , não deixo cadastrar
			if ( daoUserRepository.validarLogin(modelLogin.getLogin()) 
				...
				if ( modelLogin.isNovo()) {
					msg = "Gravado usuário com sucesso!";
				}else {
					msg = "Atualizado usuário com sucesso!";
				}
